...
class InputPolicy(object):
    ...
    def __init__(self, device, app, is_gym_policy=False):
        ...
        self.app = app
        self.master = None
        self.is_gym_policy = is_gym_policy
        ...
        while input_manager.enabled and count < input_manager.event_count:
            try:
                ...
                if count == 0 and self.master is None:
                    ...
                    if self.is_gym_policy:
                        self.logger.info("Gym policy: stopping auto generating events ")
                        return
...
class UtgGreedySearchPolicy(UtgBasedInputPolicy):
    ...
    def generate_event_based_on_utg(self):
        ...
        @return: InputEvent
        """
        self.logger.info("in input_policy, generating an event ")
...
#增加新的策略
class UtgGymPolicy(UtgBasedInputPolicy):
    """
    Gym Policy
    """
    ...
    def sort_inputs_by_humanoid(self, possible_events):
        self.current_state = self.device.get_current_state()
        if sys.version.startswith("3"):
            from xmlrpc.client import ServerProxy
        else:
    ...
    def generate_event_based_on_utg(self):
        ...
        # If there is an unexplored event, try the event first优先探索未探索事件
        for input_event in possible_events:
            if not self.utg.is_event_explored(event=input_event, state=current_state):
                self.logger.info("Trying an unexplored event.")
                self.__event_trace += EVENT_FLAG_EXPLORE
