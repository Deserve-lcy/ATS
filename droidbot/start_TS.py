        ...
        for state_i in range(max_number_of_states):
            for event_j in range(max_number_of_actions):
                if state_event_matrix[state_i, event_j] > 0:
                    ...
                    # 扁平化 new_q_values
                    new_q_values_flat = new_q_values.flatten()

                    # 获取当前状态的事件分布概率
                    current_event_distribution = state_event_matrix[state_i]

                    # 加权平均更新 Q 值
                    new_q_values_flat *= current_event_distribution[:min(new_q_values_flat.shape[0], current_event_distribution.shape[0])]

                    # 恢复成原始形状
                    new_q_values = new_q_values_flat.reshape(new_q_values.shape)
...
#     plt.show()
# def experiment():
#     bandits = [Bandit_TS(p) for p in BANDIT_PROBABILITIES]
