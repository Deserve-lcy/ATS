...
from input_policy import UtgBasedInputPolicy, UtgNaiveSearchPolicy, UtgGreedySearchPolicy, \
                         UtgReplayPolicy, UtgGymPolicy, \
                         ManualPolicy, \
                         POLICY_NAIVE_DFS, POLICY_GREEDY_DFS, \
                         POLICY_NAIVE_BFS, POLICY_GREEDY_BFS, \
                         POLICY_REPLAY, \
                         POLICY_MANUAL, POLICY_MONKEY, POLICY_NONE, POLICY_GYM

DEFAULT_POLICY = POLICY_GYM
...
class InputManager(object):
    ...
    def get_input_policy(self, device, app, master):
        ...
        elif self.policy_name == POLICY_GYM:
            self.logger.info("starting gym policy")
            input_policy = UtgGymPolicy(device, app, self.random_input, self.policy_name)
        else:
    ...
    def start(self):
        ...
        try:
            if self.policy is not None:
                if self.policy_name == POLICY_GYM:
                    self.logger.info("calling gym policy start")
                ...
                while self.enabled:
                    # when policy is none, will hang here
                    time.sleep(1)
            elif self.policy_name == POLICY_MONKEY:
